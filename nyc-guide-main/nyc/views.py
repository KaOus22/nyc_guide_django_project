...
def activity(request, activity, borough):
    if request.method == 'GET':
        return render(request=request, template_name='activity.html', context={'venues': boroughs[borough][activity].keys(), 'activity': activity, 'activities': boroughs[borough].keys()})
